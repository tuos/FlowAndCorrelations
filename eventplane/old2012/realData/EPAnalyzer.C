...
void EPAnalyzer(){
  ...
  double minptcut[NB_SLICE_PT] = {0.3,0.4,0.6,0.8,1.1,1.4,1.75,2.2,2.75,3.45,4.2,5.05,6.0,7.2,8.5};
...
double phimpsi32=0;
     ...
     t1.Add("/scratch/appelte1/STEG_run182798_hiGeneralAndPixelTracks_v0.root/ana/tree");
     t1.SetBranchAddress("npg", &rmult);
     t1.SetBranchAddress("cent", &rcent);
    ...
    TFile *f = new TFile("phipsi_dist.root","recreate");
     ...
     for(long ne=0; ne<neve; ne++)
     {
       ...
       t1.GetEntry(ne);
       //cout<<"cent = "<<rcent<<endl;
       //if(rmult<110) continue;
       if(rcent<60||rcent>69) continue;
       Neve++;
       Nh=0;  
