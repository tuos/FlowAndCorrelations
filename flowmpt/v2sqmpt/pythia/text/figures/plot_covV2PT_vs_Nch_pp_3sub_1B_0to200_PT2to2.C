void plot_covV2PT_vs_Nch_pp_3sub_1B_0to200_PT2to2() {
  const int nMultBin=330;
  double temp;
  ...
  in1.open("./output_covV2PT_vs_Nch_3sub_pp_PT3to2_NEWAC.txt");
...
////////
 ...
 gStyle->SetOptStat(0);
  c1->SetTicks(-1);
 TH1D* hist = new TH1D("hist","",330,0.,330.);
 hist->SetXTitle("N_{ch}");
 hist->SetYTitle("cov(v_{2}^{2},[p_{T}])");
 //hist->SetMinimum(-0.001);
 //hist->SetMaximum(0.0005);
 hist->SetMinimum(-0.0002);
 hist->SetMaximum(0.0005);
...
/*  TGraphErrors *gr3 = new TGraphErrors(20,mult_LOOPCUT,cov_LOOPCUT,0,cov_LOOPCUT_error);
  ...
  gr1->Draw("psameezL");
*/
  TGraphErrors *gr2 = new TGraphErrors(nMultBin-50,mult_3SUB,cov_3SUB,0,cov_3SUB_error);
  gr2->SetTitle("");
  ...
  gr2->Draw("psameezL");
    ...
    TLegend *leg = new TLegend(0.8,0.8,0.8,.8);
    leg->SetFillColor(10);
    leg->SetBorderSize(0);
    leg->SetTextFont(42);
    leg->SetTextColor(1);
    ...
    leg->AddEntry(gr2,"3 Sub-event","pl");
