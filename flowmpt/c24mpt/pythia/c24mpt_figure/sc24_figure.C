void sc24_figure(){
  ...
  for(int k =0; k<NMultBin; k++){
    ...
    cov1[k]=hc24mpt1[k]->GetRMS();
    cov2[k]=hc24mpt2[k]->GetRMS();
    cov3[k]=hc24mpt3[k]->GetRMS();
    cov4[k]=hc24mpt4[k]->GetRMS();
    cove1[k]=hc24mpt1[k]->GetRMSError();
    cove2[k]=hc24mpt2[k]->GetRMSError();
    cove3[k]=hc24mpt3[k]->GetRMSError();
    cove4[k]=hc24mpt4[k]->GetRMSError();
  }
 ...
 //hist->Draw();
        ...
        hist3->SetYTitle("#sigma_{c_{2}#{}{4}}");
        hist3->SetMinimum(-0.0000001);
        hist3->SetMaximum(0.0015);
        hist3->Draw();
        ...
        TLatex *tex = new TLatex(12,0.00132,"PYTHIA8 13 TeV");
        tex->SetTextSize(0.04);
        tex->SetLineWidth(2);
        tex->SetTextFont(42);
        tex->Draw();
        tex = new TLatex(12,0.0012,"0.3 < p_{T} < 2 GeV");
        tex->SetTextSize(0.04);
        tex->SetLineWidth(2);
        tex->SetTextFont(42);
        tex->Draw();
