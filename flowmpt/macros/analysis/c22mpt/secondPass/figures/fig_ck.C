void fig_ck() {
  ...
  for(int i=0;i<nMultBin-1;i++){
    ...
    rho_2[i] = CK_2[i+1]->GetMean();

   rhoe_2[i]=hckerr[i+1]->GetRMS()/sqrt(hckerr[i+1]->GetEntries()); 
cout<<Nch_2[i]<<"   "<<rho_2[i]<<"   "<<rhoe_2[i]<<endl;
  ...
  cout<<endl;
  ofstream outFile;
  outFile.open("out_ck_c22_pbpb.txt");
  for(int i=0;i<nMultBin-1;i++){
    cout<<Nch_2[i]<<"   "<<rho_2[i]<<"   "<<rhoe_2[i]<<endl;
...
////////
 ...
 hist->SetYTitle("c_{k} [GeV^{2}]");
 //hist->SetMinimum(-0.001);
 //hist->SetMaximum(0.0005);
 hist->SetMinimum(0.00099);
 hist->SetMaximum(0.021);
 ...
 hist->Draw();
  ...
  //gr4->Draw("psameezL");
    ...
    TLatex *tex1 = new TLatex(122,0.016,"PbPb 5 TeV");
    tex1->SetTextColor(1);
    ...
    //tex2x->Draw();
    TLatex *tex3 = new TLatex(0,0.022,"CMS Preliminary");
    tex3->SetTextColor(1);
