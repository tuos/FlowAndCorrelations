void fig_cov() {
  ...
  for(int i=0;i<nMultBin-1;i++){
   ...
   rhoe_2[i]=hcov22err[i+1]->GetRMS()/sqrt(hcov22err[i+1]->GetEntries()); 
cout<<Nch_2[i]<<"   "<<rho_2[i]<<"   "<<rhoe_2[i]<<endl;
  ...
  cout<<endl;
  ofstream outFile;
  outFile.open("out_cov_c22_pbpb.txt");
  for(int i=0;i<nMultBin-1;i++){
    cout<<Nch_2[i]<<"   "<<rho_2[i]<<"   "<<rhoe_2[i]<<endl;
...
////////
 ...
 hist->SetYTitle("cov(c_{2}{2},[p_{T}]) [GeV]");
 hist->SetMinimum(-0.000199);
 hist->SetMaximum(0.000199);
 ...
 hist->Draw();
  ...
  //gr4->Draw("psameezL");
    ...
    TLatex *tex1 = new TLatex(122,0.00016,"PbPb 5 TeV");
    tex1->SetTextColor(1);
    ...
    tex2x->Draw();
    TLatex *tex3 = new TLatex(30,0.000207,"CMS Preliminary");
    tex3->SetTextColor(1);
  ...
  c1->Print("plot_fig_cov22_tmp.pdf");
