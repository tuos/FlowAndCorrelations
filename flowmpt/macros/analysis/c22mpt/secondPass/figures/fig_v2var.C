void fig_v2var() {
  ...
  for(int i=0; i<nJob; i++){
    for(int k =0; k<nMultBin; k++){
       ...
       if(!isnan(Corr4_2i[i][k]->GetMean() - pow(Corr2_2i[i][k]->GetMean(),2))) hv2varerr[k]->Fill(Corr4_2i[i][k]->GetMean() - pow(Corr2_2i[i][k]->GetMean(),2));
  ...
  for(int i=0;i<nMultBin-1;i++){
    ...
    rho_2[i] = (Corr4_2[i+1]->GetMean() - pow(Corr2_2[i+1]->GetMean(),2)); 
    if(rho_2[i]<0) rho_2[i]=-rho_2[i];
   rhoe_2[i]=hv2varerr[i+1]->GetRMS()/sqrt(hv2varerr[i+1]->GetEntries()); 
cout<<Nch_2[i]<<"   "<<rho_2[i]<<"   "<<rhoe_2[i]<<endl;
  ...
  cout<<endl;
  ofstream outFile;
  outFile.open("out_v2var_c22_pbpb.txt");
  for(int i=0;i<nMultBin-1;i++){
    cout<<Nch_2[i]<<"   "<<rho_2[i]<<"   "<<rhoe_2[i]<<endl;
...
////////
 ...
 hist->SetYTitle("Var(c_{2}{2})_{dyn}");
 //hist->SetMinimum(-0.001);
 //hist->SetMaximum(0.0005);
 hist->SetMinimum(-0.00001);
 hist->SetMaximum(0.000299);
 ...
 hist->Draw();
  ...
  //gr4->Draw("psameezL");
    ...
    TLatex *tex1 = new TLatex(122,0.00027,"PbPb 5 TeV");
    tex1->SetTextColor(1);
    ...
    tex2x->Draw();
    TLatex *tex3 = new TLatex(30,0.000305,"CMS Preliminary");
    tex3->SetTextColor(1);
    ...
    leg->AddEntry(gr2,"0.5 < p_{T} < 5 GeV","pl");
  ...
  c1->Print("plot_fig_v2var_tmp.pdf");
