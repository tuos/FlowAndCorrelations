...
TH1D* hNtrkOffline[nCentBin];
  ...
  TFile *inFile = new TFile("merged.root","read");
...
//draw 
 ...
 hist->GetYaxis()->SetNdivisions(508);
  ...
  TGraphErrors *graph24 = new TGraphErrors(nCentBin-1,vcent,v24Values,0,v24Errors);
    ...
    leg->AddEntry(graph22,"v_{2}{2, no gap}","pl");
    //leg->AddEntry(grAB00,"v_{2}{2, gap>0}","pl");
    //leg->AddEntry(grAB05,"v_{2}{2, gap>1}","pl");
    leg->AddEntry(grAB10,"v_{2}{2, gap>2}","pl");
    //leg->AddEntry(grAB15,"v_{2}{2, gap>3}","pl");
    leg->AddEntry(graph24,"v_{2}{4}","pl");
    leg->Draw();

    tex->DrawLatex(25, 0.85*hist->GetMaximum(), Form("OO 5.36 TeV"));

  c1->SaveAs("plot_v2qc_cent.png");
  c1->SaveAs("plot_v2qc_cent.pdf");
