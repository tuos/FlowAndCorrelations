void countsVsvz_pt10_15(){
  ...
  ifstream incounts;
  incounts.open("data_counts_pt10_15_N4.txt");
  ...
  incounts.close();
  incounts.open("data_counts_pt10_15_S4.txt");
  for(int i=0;i<nVzBin;i++){
    incounts>>vzS4[i];
    incounts>>countsS4[i];
  ...
  incounts.close();
  incounts.open("data_counts_pt10_15_N23.txt");
  for(int i=0;i<nVzBin;i++){
    incounts>>vzN23[i];
  ...
  incounts.close();
  incounts.open("data_counts_pt10_15_S23.txt");
  for(int i=0;i<nVzBin;i++){
    incounts>>vzS23[i];
...
////////
 ...
 hist->Draw();
  ...
  countsVsVz->Draw("same");
    ...
    TLatex *tex2= new TLatex(-6,countMax*1.03,"0-10%         1.0<p_{T}<1.5 GeV");
    tex2->SetTextColor(1);
  ...
  c1->Print("plot_countsVsvz_pt10_15.png");
  c1->Print("plot_countsVsvz_pt10_15.pdf");
