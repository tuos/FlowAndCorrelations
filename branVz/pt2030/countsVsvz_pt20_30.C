void countsVsvz_pt20_30(){
  ...
  ifstream incounts;
  incounts.open("data_counts_pt20_30_N4.txt");
  ...
  incounts.close();
  incounts.open("data_counts_pt20_30_S4.txt");
  for(int i=0;i<nVzBin;i++){
    incounts>>vzS4[i];
    incounts>>countsS4[i];
  ...
  incounts.close();
  incounts.open("data_counts_pt20_30_N23.txt");
  for(int i=0;i<nVzBin;i++){
    incounts>>vzN23[i];
  ...
  incounts.close();
  incounts.open("data_counts_pt20_30_S23.txt");
  for(int i=0;i<nVzBin;i++){
    incounts>>vzS23[i];
...
////////
 ...
 hist->Draw();
  ...
  countsVsVz->Draw("same");
    ...
    TLatex *tex2= new TLatex(-6,countMax*1.03,"0-10%         2.0<p_{T}<3.0 GeV");
    tex2->SetTextColor(1);
  ...
  c1->Print("plot_countsVsvz_pt20_30.png");
  c1->Print("plot_countsVsvz_pt20_30.pdf");
