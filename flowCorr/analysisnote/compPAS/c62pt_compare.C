...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../outchi6222D_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../Stevedata/mc6222m.txt");
  for(int i=0; i<nw; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  can->cd();
  can->Print("./figures/c62_pt_12cent.png");
  can->Print("./figures/c62_pt_12cent.pdf");
}
