...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../outchi7223D_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../Stevedata/mc7223m.txt");
  for(int i=0; i<nw; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  for (int iw=0;iw<nw-2;iw++)
  {
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*3.3,lymin[iw]*0.35,lxmax[iw]*2.2,lymax[iw]*0.6);
  ...
  can->cd();
  can->Print("./figures/c723_pt_12cent.png");
  can->Print("./figures/c723_pt_12cent.pdf");
}
