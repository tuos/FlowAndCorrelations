...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="#chi_{633}";
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../outchi633D_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../Stevedata/mc633m.txt");
  for(int i=0; i<nw; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  can->cd();
  can->Print("./figures/c63_pt_12cent.png");
  can->Print("./figures/c63_pt_12cent.pdf");
}
