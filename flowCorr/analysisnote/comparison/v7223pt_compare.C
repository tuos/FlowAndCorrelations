...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../../v01Aug15_5tev/outv723_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../outv723_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+4; j++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{7}{#Psi_{223}} 5.02 TeV","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{7}{#Psi_{223}} 2.76 TeV","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v7223_pt_10cen.png");
  can->Print("./figures/v7223_pt_10cen.pdf");
}
