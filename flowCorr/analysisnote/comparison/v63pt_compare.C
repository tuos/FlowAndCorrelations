...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../../v01Aug15_5tev/outv63_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{6}{#Psi_{33}} 5.02 TeV","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{6}{#Psi_{33}} 2.76 TeV","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v63_pt_10cen.png");
  can->Print("./figures/v63_pt_10cen.pdf");
}
