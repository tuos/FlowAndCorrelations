...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      ...
      //tex->Draw();
      tex=new TLatex(5,0.00033,"|#eta|<0.8");
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*0.6,lymin[iw]*1.16,lxmax[iw]*0.6,lymax[iw]*1.02);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"CMS v_{n}, 5.02 TeV","pl");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*5+iw;
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"CMS v_{n}, 2.76 TeV","pl");
    }
    if (iw==2) leg->Draw();
  ...
  if(iw==1){
    for (int im=0;im<1;im++)
    {
      ...
      ge->Draw("le");
      TLegend *leg = new TLegend(lxmin[iw]*0.6,lymin[iw]*0.6,lxmax[iw]*0.6,lymax[iw]*0.6);
      leg->SetTextSize(textsize);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      //ge->SetFillColor(kYellow+2);
      ge->Draw("le");
      leg->AddEntry(ge,"2.76 TeV, estimate","l");
    }
  }
  if(iw==3){
    {
      ...
      //ge->SetFillColor(kYellow+2);
      ge->Draw("le");
      TLegend *leg = new TLegend(lxmin[iw]*0.6,lymin[iw]*1.1,lxmax[iw]*0.6,lymax[iw]*1.1);
      leg->SetTextSize(textsize);
      leg->AddEntry(ge,"2.76 TeV, estimate","l");
      leg->Draw();
    }
  ...
  can->cd();
  can->Print("./figures/vn_cent_theorycomp.png");
  can->Print("./figures/vn_cent_theorycomp.pdf");
}
