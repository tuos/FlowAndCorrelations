...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=0.0001; hymax[ip]=0.099;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=5.0; ptit[ip][1]=0.08;// position legends 
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../../v01Aug15_5tev/outv523_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../../v01Aug18_generalTracks/outv523_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+4; j++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      tex=new TLatex(1.1,0.08,"CMS PbPb #sqrt{s_{NN}}=5.02TeV");
    ...
    if (iw < 4) tex=new TLatex(ptit[iw][0]+2,0.07,htit[iw]);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{5}{#Psi_{23}} hiGeneralAndPixelTracks","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{5}{#Psi_{23}} hiGeneralTracks","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v523pt_compare_pixel_general.png");
}
