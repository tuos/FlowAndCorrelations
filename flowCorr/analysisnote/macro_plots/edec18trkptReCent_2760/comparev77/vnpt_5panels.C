...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=-0.0005; hymax[ip]=0.0463*2;
    lxmin[ip]=0.24; lxmax[ip]=0.88;
    lymin[ip]=0.65; lymax[ip]=0.94;
    ptit[ip][0]=0.4; ptit[ip][1]=0.036;// position legends 
  ...
  const int npt_oli=8;
  ifstream inEP;
  inEP.open("./data_vnptcent550_2760gev.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
  ...
  inEP.close();
  ifstream inEP;
  inEP.open("./data_vnptcent550_5020gev.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
  ...
  inEP.close();
  ifstream inEP;
  inEP.open("./data_vnptcent550_5020gev_generalTracks.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    h1[iw]->GetXaxis()->SetNdivisions(606);
      ...
      h1[iw]->GetYaxis()->SetTitleOffset(1.9);
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==3)
    {
      tex=new TLatex(1.5,0.0235*0.88*1.275*2,"5-50%,  |#eta|<2.4");
    ...
    tex=new TLatex(ptit[iw][0],ptit[iw][1]*1.125*2,htit[iw]);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      //ge->Draw("pe");
      leg->AddEntry(ge,"5.02 TeV, v_{n}\{#Psi_{xyz}\}","pl");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*5+iw;
      ...
      //ge2->Draw("pe");
      leg->AddEntry(ge2,"5.02 TeV, v_{n}\{#Psi_{n}\}","pl");
  ...
  can->cd();
  can->Print("./figures/plot_vnpt_5panels.png");
  can->Print("./figures/plot_vnpt_5panels.pdf");
}
