...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt_oli=8;
  ifstream inEP;
  inEP.open("./data_default_v7223cent.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
  ...
  inEP.close();
  ifstream inEP;
  inEP.open("./data_correctselfcorr_v7223cent.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
      //inEP>>tmp; inEP>>tmp; inEP>>xa[i][j]; inEP>>tmp; inEP>>ya[i][j]; inEP>>ye[i][j];
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if(iw==0){
     h1[iw]->SetYTitle("v_{7}{#Psi_{223}}");
     h1[iw]->SetMinimum(0.0000); 
     h1[iw]->SetMaximum(0.0069);
      h1[iw]->GetYaxis()->SetTitleOffset(1.65);
    }
    if(iw==1){
     h1[iw]->SetYTitle("#chi_{7223}");
     h1[iw]->SetMinimum(-1); 
     h1[iw]->SetMaximum(17);
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==1)
    {
      ...
      //tex->Draw();
      tex=new TLatex(10,6.1033,"|#eta|<2.4");
  ...
  can->cd();
  can->Print("./figures/plot_selfvnchicent7223.png");
  can->Print("./figures/plot_selfvnchicent7223.pdf");
}
