...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{6}{EP} (HIN-11-005)","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{6}{SP} (new analysis)","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v62_pt_10cen.png");
  can->Print("./figures/v62_pt_10cen.pdf");
}
