...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{2}{#Psi_{2}}";
  }
  //
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw < 4) tex=new TLatex(ptit[iw][0]+0,0.27,htit[iw]);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{2}{EP} (HIN-10-002)","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{2}{SP} (new analysis)","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v2_pt_12cen.png");
  can->Print("./figures/v2_pt_12cen.pdf");
}
