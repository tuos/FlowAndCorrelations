...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../../edec18trkQReCent_5020/outchi633_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../outchi633_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      ...
      //tex->Draw();
      tex=new TLatex(1.5,7.07,"|#eta|<2.4");
      tex->SetTextSize(textsize);
      tex->Draw();
    }
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"#chi_{633} 5.02 TeV","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"#chi_{633} 2.76 TeV","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/chi633_pt_10cen.png");
  can->Print("./figures/chi633_pt_10cen.pdf");
}
