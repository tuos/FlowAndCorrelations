...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{3}{#Psi_{3}}";
  }
  //
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      ...
      tex->Draw();
      tex=new TLatex(1.5,0.30,"|#eta|<2.4");
      tex->SetTextSize(textsize);
      tex->Draw();
    }
    if (iw < 4) tex=new TLatex(ptit[iw][0]+1,0.27,htit[iw]);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{3}{EP} (HIN-11-005)","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{3}{SP} (new analysis)","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v3_pt_11cen.png");
  can->Print("./figures/v3_pt_11cen.pdf");
}
