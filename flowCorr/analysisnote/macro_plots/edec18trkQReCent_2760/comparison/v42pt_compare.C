...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=0.0001; hymax[ip]=0.199;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=5.0; ptit[ip][1]=0.14;// position legends 
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      tex=new TLatex(1.1,0.15,"CMS PbPb #sqrt{s_{NN}}=2.76TeV");
      tex->SetTextSize(textsize*0.85);
      tex->Draw();
      tex=new TLatex(1.5,0.10,"|#eta|<2.4");
      tex->SetTextSize(textsize);
      tex->Draw();
    }
    if (iw < 4) tex=new TLatex(ptit[iw][0]+1,0.07,htit[iw]);
    else tex=new TLatex(ptit[iw][0]+1,ptit[iw][1],htit[iw]);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{4}{EP} (HIN-11-005)","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{4}{SP} (new analysis)","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v42_pt_10cen.png");
  can->Print("./figures/v42_pt_10cen.pdf");
}
