...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=15;
  ifstream inEP;
  inEP.open("../../edec18trkQReCent_5020/outv523_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../../edec18trkQReCent_2760/outv523_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      ...
      //tex->Draw();
      tex=new TLatex(1.5,0.07,"|#eta|<2.4");
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{5}{#Psi_{23}} 5.02 TeV","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{5}{#Psi_{23}} 2.76 TeV","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v523_pt_10cen.png");
  can->Print("./figures/v523_pt_10cen.pdf");
}
