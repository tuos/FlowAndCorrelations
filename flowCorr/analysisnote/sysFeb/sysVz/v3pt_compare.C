...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{3}{#Psi_{3}} ratio";
  }
  //
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp2/figures_-0.8_00.8/outv33steve.txt");
  //inSP.open("../aug13L/outv33_corrFlow.txt");
  for(int i=0; i<nw; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  inSP.close();
/*
  inSP.open("../aug13T/outv33_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
...
*/
  ...
  can->cd();
  can->Print("./figures/v3_pt_12cent.png");
  can->Print("./figures/v3_pt_12cent.pdf");
}
