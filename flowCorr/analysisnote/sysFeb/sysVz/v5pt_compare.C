...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{5}{#Psi_{5}} ratio";
  }
  //
  ...
  ifstream inSP;
  //inSP.open("../MH_noRecenter_20Oct2017/N523ASUB3/data/v52.txt");
  inSP.open("/Users/shengquantuo/Downloads/tmp2/figures_-0.8_00.8/outv55steve.txt");
  //inSP.open("../../../sep16/figures_tight_2/N523ASUB3/data/v523.txt");
  //inSP.open("../../sep15/recenter_Mix/outchi523Num_corrFlow.txt");
  //inSP.open("../../../aug13/aug13T/outv523_corrFlow.txt");
  for(int i=0; i<nw; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  inSP.close();
/*
  inSP.open("../../sep15/recenter/outchi523Num_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
...
*/
  ...
  for (int iw=0;iw<nw-2;iw++)
  {
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*3.3,lymin[iw]*1.3,lxmax[iw]*2.2,lymax[iw]*1.6);
...
/*
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge," Old Num/New","p");
    }
*/
  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v5_pt_12cent.png");
  can->Print("./figures/v5_pt_12cent.pdf");
}
