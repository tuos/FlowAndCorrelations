...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{4}{#Psi_{2}} ratio";
  }
  //
  ...
  const int npt=15;
  ifstream inEP;
  //inEP.open("../../aug13/aug13BT/outv42_corrFlow.txt");
  inEP.open("../outv42_corrFlow.txt");
  //inEP.open("../outchi422Num_corrFlow.txt");
  //inEP.open("../../../aug13/aug13BN/outv42_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp2/figures_-0.8_00.8/outv422Asteve.txt");
  //inSP.open("./steve.txt");
  //inSP.open("../../../aug13/aug13BT/outv42_corrFlow.txt");
  for(int i=0; i<nw; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  inSP.close();
/*
  inSP.open("../aug13T/outv42_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
...
*/
  ...
  for (int iw=0;iw<nw-2;iw++)
  {
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*3.3,lymin[iw]*1.1,lxmax[iw]*2.2,lymax[iw]*1.3);
...
*/
  ...
  can->cd();
  can->Print("./figures/v42_pt_12cent.png");
  can->Print("./figures/v42_pt_12cent.pdf");
}
