...
//20-60%
  ...
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
      ...
      nBRye60[i][j]=nBya60[i][j]/ya60[i][j]*sqrt(ye60[i][j]*ye60[i][j]/ya60[i][j]/ya60[i][j] + nBye60[i][j]*nBye60[i][j]/nBya60[i][j]/nBya60[i][j] - 2.0*ye60[i][j]*ye60[i][j]/ya60[i][j]/nBya60[i][j] );
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    for (int im=0;im<1;im++)
    {
      ...
      //ge2->Draw("pe");
      leg->AddEntry(ge2,"3<|vz|<15/|vz|<15","pl");
...
cout<<fitFun->Eval(0.4)<<",  "<<fitFun->Eval(0.9)<<",  "<<fitFun->Eval(1.9)<<",  "<<fitFun->Eval(3.5)<<endl;
   ...
   tex->Draw("same");
    ...
    for (int im=0;im<1;im++)
    {
      ...
      //ge3->Draw("pe");
      leg2->AddEntry(ge3,"|vz|<3/|vz|<15","pl");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*5+iw;
      ...
      //ge4->Draw("pe");
      leg2->AddEntry(ge4,"3<|vz|<15/|vz|<15","pl");
    }
    ge4->Draw("pe");
    ge3->Draw("pe");
    TLine *lines = new TLine(0,1,4.0,1);
...
cout<<fitFun->Eval(2.5)<<",  "<<fitFun->Eval(7.5)<<",  "<<fitFun->Eval(45)<<",  "<<fitFun->Eval(55)<<endl;
  ...
  can->cd();
  can->Print("./figures/plot_vnpt_5panels_vzSys_ratio.png");
  can->Print("./figures/plot_vnpt_5panels_vzSys_ratio.pdf");
