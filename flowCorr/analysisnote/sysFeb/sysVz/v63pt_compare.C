...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{6}{#Psi_{3}} ratio";
  }
  //
  ...
  ifstream inSP;
  //inSP.open("./Stevedata/mv2m.txt");
  inSP.open("../../../../../v1/ajan16trkQReCent_5020/outv63_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
  ...
  inSP.close();
/*
  inSP.open("../aug13T/outv63_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
...
*/
  ...
  can->cd();
  can->Print("./figures/v63_pt_12cent.png");
  can->Print("./figures/v63_pt_12cent.pdf");
}
