...
//start new
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      //ge->Draw("pe");
      leg2->AddEntry(ge,"|vz|<3/|vz|<15","pl");
    }
    for (int im=0;im<1;im++)
    { 
      int j=im*5+iw;
      ...
      geB->SetLineColor(col);
      //ge->Draw("pe");
      leg2->AddEntry(geB,"3<|vz|<15/|vz|<15","pl");
    }
    ge->Draw("pe");
    geB->Draw("pe");
    if (iw==2) leg2->Draw();
   ...
   TLatex *tex = new TLatex(10.06,0.7,Form("fit p0 = %4.3f #pm %4.3f", p0, p0Err));
   tex->SetTextColor(2);
   tex->SetTextSize(16);
   tex->Draw("same");
...
//end new
  ...
  can->cd();
  can->Print("./figures/plot_chicent_vzSys_ratio.png");
  can->Print("./figures/plot_chicent_vzSys_ratio.pdf");
