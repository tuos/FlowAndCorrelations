...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{2}{#Psi_{2}} ratio";
  }
  //
  ...
  const int npt=15;
  ifstream inEP;
  //inEP.open("../../aug13/aug13BT/outv22_corrFlow.txt");
  inEP.open("../outv22_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp2/figures_-0.8_00.8/outv22steve.txt");
  //inSP.open("../../../sep26/sep26B/outv22_corrFlow.txt");
  //inSP.open("../../../aug13/aug13BT/outv22_corrFlow.txt");
  for(int i=0; i<nw; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  inSP.close();
/*
  inSP.open("../aug13T/outv22_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+5; j++){
...
*/
  ...
  can->cd();
  can->Print("./figures/v2_pt_12cent.png");
  can->Print("./figures/v2_pt_12cent.pdf");
}
