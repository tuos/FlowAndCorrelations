...
nAya[i][j]=nARya[i][j]*ya[i][j]; //needed for the top panels!
  ...
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
      ...
      nBRxa[i][j]=nBxa[i][j];
      nBRxe[i][j]=nBxe[i][j];
      nBRye[i][j]=nBya[i][j]/ya[i][j]*sqrt(ye[i][j]*ye[i][j]/ya[i][j]/ya[i][j] + nBye[i][j]*nBye[i][j]/nBya[i][j]/nBya[i][j] - 1.6*ye[i][j]*ye[i][j]/ya[i][j]/nBya[i][j] );
...
//end new
  ...
  can->cd();
  can->Print("./figures/plot_chicent_esSys_ratio.png");
  can->Print("./figures/plot_chicent_esSys_ratio.pdf");
