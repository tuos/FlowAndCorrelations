...
double lymin[Mpads],lymax[Mpads];
  ...
  inEP.close();
  ifstream inEP;
  inEP.open("./data_vncent5020_integral_hfA.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
  ...
  inEP.close();
  ifstream inEP;
  inEP.open("./data_vncent5020_integral_hfB.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt; j++){
...
//end new
  ...
  can->cd();
  can->Print("./figures/plot_vncent_hfSys_ratio.png");
  can->Print("./figures/plot_vncent_hfSys_ratio.pdf");
