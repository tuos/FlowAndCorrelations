...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=0.0001; hymax[ip]=2.099;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=5.0; ptit[ip][1]=8.08;// position legends 
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="#chi_{4}{#Psi_{22}}";
  }
  //
...
sysEach[i]=0.02;
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp3/figures_-0.8_00.8/outc523steve.txt");
  for(int i=0; i<nw-2; i++){
      //inSP>>tmp; inSP>>tmp;
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw < 4) tex=new TLatex(ptit[iw][0]+2,7.07,htit[iw]);
  ...
  can->cd();
  can->Print("./figures/chi523_pt_10cen_shengquansteve.png");
  can->Print("./figures/chi523_pt_10cen_shengquansteve.pdf");
}
