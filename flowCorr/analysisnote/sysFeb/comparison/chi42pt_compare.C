...
double nxesys[Mfiles][Mpoints], nyesys[Mfiles][Mpoints];
...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=0.0001; hymax[ip]=3.099;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=5.0; ptit[ip][1]=2.608;// position legends 
  ...
  const int npt=16;
  ifstream inEP;
  inEP.open("../outchi422D_corrFlow.txt");
...
sysEach[i]=0.02;
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp3/figures_-0.8_00.8/outc422steve.txt");
  for(int i=0; i<nw-2; i++){
      //inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+2; j++){
      ...
      nxe[i][j]=0.0;
      nxesys[i][j]=0.2;
      nyesys[i][j]=sqrt(sysEach[i]*sysEach[i] + (nya[i][j]*sysXYZ[i]*0.01)*(nya[i][j]*sysXYZ[i]*0.01));
    }
  }
  inSP.close();
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw],lymin[iw]*1.3,lxmax[iw],lymax[iw]*1.3);
    leg->SetFillColor(0);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      int col=2;
      //if (ndp[j]==0) continue;
      ge=new TGraphErrors(18,&nxa[j][0],&nya[j][0],&nxe[j][0],&nye[j][0]);
      ge->SetTitle("");
  ...
  can->cd();
  can->Print("./figures/chi42_pt_10cen_shengquansteve.png");
  can->Print("./figures/chi42_pt_10cen_shengquansteve.pdf");
}
