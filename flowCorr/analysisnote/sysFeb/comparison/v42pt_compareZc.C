...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=-0.00605; hymax[ip]=0.00619;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=4.0; ptit[ip][1]=0.005;// position legends 
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{4}{#Psi_{22}} diff";
  }
  //
...
sysEach[i]=0.0002;
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp3/figures_-0.8_00.8/outv422Asteve.txt");
  for(int i=0; i<nw-2; i++){
...
ya[i][j]=0;
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw < 4) tex=new TLatex(ptit[iw][0]*1.2,ptit[iw][1]*1.,htit[iw]);
    else tex=new TLatex(ptit[iw][0]*1.2,ptit[iw][1]*1.,htit[iw]);
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*3.99,lymin[iw]*1.6,lxmax[iw]*1.99,lymax[iw]*1.3);
    leg->SetFillColor(0);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge2=new TGraphErrors(16,&mxa[j][0],&ya[j][0],&mxe[j][0],&mye[j][0]);
      drawSysBoxValue(ge2,16,0.2, myesys[j]);
      ge->SetTitle("");
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ///ge->Draw("pe");
      ///leg->AddEntry(ge,"v_{4}{#Psi_{22}} Shengquan","p");
    }
    if (iw==0) leg->Draw();
  ...
  can->cd();
  can->Print("./figures/v42_pt_ShengquanStevec.png");
  can->Print("./figures/v42_pt_ShengquanStevec.pdf");
}
