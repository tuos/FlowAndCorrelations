...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=0.0001; hymax[ip]=0.119;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=4.0; ptit[ip][1]=0.08;// position legends 
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{4}{#Psi_{22}}";
  }
  //
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*0.6,lymin[iw],lxmax[iw]*0.6,lymax[iw]*1.3);
    leg->SetFillColor(0);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{4}{#Psi_{22}}(Sanders,|#eta|<0.8)","p");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*12+iw;
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{4}{#Psi_{22}} (Tuo, |#eta|<0.8)","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{4}{#Psi_{22}} (11005, |#eta|<0.8)","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v42_pt_HIN11005_5TeVShengquanSteve.png");
  can->Print("./figures/v42_pt_HIN11005_5TeVShengquanSteve.pdf");
}
