...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hxmin[ip]=0.000; hxmax[ip]=8.3;
    hymin[ip]=0.0001; hymax[ip]=0.079;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=2.1; ptit[ip][1]=0.04;// position legends 
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp2/figures_-0.8_00.8/outv523steve.txt");
  for(int i=0; i<nw-2; i++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      tex=new TLatex(0.5,0.046,"CMS PbPb #sqrt{s_{NN}}=5.02TeV");
      tex->SetTextSize(textsize*0.85);
      tex->Draw();
      tex=new TLatex(0.3,0.035,"|#eta|<0.8");
      tex->SetTextSize(textsize);
      //tex->Draw();
    }
    //if (iw < 4) tex=new TLatex(ptit[iw][0]+0.0,0.06,htit[iw]);
    //else tex=new TLatex(ptit[iw][0],ptit[iw][1],htit[iw]);
    tex=new TLatex(ptit[iw][0]+0.0,0.06,htit[iw]);
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*0.5,lymin[iw],lxmax[iw]*0.6,lymax[iw]*1.1);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{5}{#Psi_{23}}(Sanders,|#eta|<0.8)","p");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*12+iw;
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{5}{#Psi_{23}} (Tuo, |#eta|<0.8)","p");
    }
    if (iw==1) leg->Draw();
  ...
  can->cd();
  can->Print("./figures/v523_pt_5TeVShengquanSteve.png");
  can->Print("./figures/v523_pt_5TeVShengquanSteve.pdf");
}
