...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=16;
  ifstream inEP;
  inEP.open("../../tight/outchi633_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../../loose/outchi633_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+4; j++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"#chi_{633} (tight)","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"#chi_{633} (loose)","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/chi633_pt_10cen_sysTrackCuts2760gev.png");
  can->Print("./figures/chi633_pt_10cen_sysTrackCuts2760gev.pdf");
}
