...
double lymin[Mpads],lymax[Mpads];
  ...
  const int npt=20;
  ifstream inEP;
  inEP.open("./ajan09trkQReCent_5020/outv42_corrFlow.txt");
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      tex=new TLatex(1.1,0.09,"CMS PbPb #sqrt{s_{NN}}=2.76TeV");
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{4}{#Psi_{22}} (Tuo)","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{4}{#Psi_{22}} (Steve)","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/v42_pt_10cen_sysTrackCuts2760gev.png");
  can->Print("./figures/v42_pt_10cen_sysTrackCuts2760gev.pdf");
}
