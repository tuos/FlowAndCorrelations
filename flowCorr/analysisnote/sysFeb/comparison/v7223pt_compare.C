...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hymin[ip]=-0.001; hymax[ip]=0.029;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=5.0; ptit[ip][1]=0.02;// position legends 
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="v_{7}{#Psi_{223}}";
  }
  //
  ...
  ifstream inSP;
  inSP.open("/Users/shengquantuo/Downloads/tmp2/figures_-0.8_00.8/outv7223steve.txt");
  for(int i=0; i<nw-2; i++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      ...
      //tex->Draw();
      tex=new TLatex(1.5,0.035,"|#eta|<0.8");
      tex->SetTextSize(textsize);
      //tex->Draw();
    }
    if (iw < 4) tex=new TLatex(ptit[iw][0]*1.2,ptit[iw][1]*1.2,htit[iw]);
    else tex=new TLatex(ptit[iw][0]*1.2,ptit[iw][1]*1.2,htit[iw]);
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*0.6,lymin[iw],lxmax[iw]*0.6,lymax[iw]);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{7}{#Psi_{223}}(Sanders,|#eta|<0.8)","p");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*12+iw;
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"v_{7}{#Psi_{223}} (Tuo, |#eta|<0.8)","p");
    }
    if (iw==1) leg->Draw();
  ...
  can->cd();
  can->Print("./figures/v7223_pt_5TeVShengquanSteve.png");
  can->Print("./figures/v7223_pt_5TeVShengquanSteve.pdf");
}
