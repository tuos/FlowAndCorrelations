...
double lymin[Mpads],lymax[Mpads];
  ...
  for (int ip=0;ip<Mpads;ip++)
  {
    ...
    hxmin[ip]=0.0001; hxmax[ip]=3.5;
    hymin[ip]=0.0001; hymax[ip]=50.099;
    lxmin[ip]=0.08; lxmax[ip]=0.35;
    lymin[ip]=0.4; lymax[ip]=0.65;
    ptit[ip][0]=2.1; ptit[ip][1]=4.08;// position legends 
    atit[ip][0]="p_{T} (GeV/c)"; atit[ip][1]="#chi_{5}{#Psi_{23}}";
  }
  //
  ...
  const int npt=16;
  ifstream inEP;
  inEP.open("../../tight/outchi523_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inEP>>tmp; inEP>>tmp;
  ...
  ifstream inSP;
  inSP.open("../../loose/outchi523_corrFlow.txt");
  for(int i=0; i<nw; i++){
      inSP>>tmp; inSP>>tmp;
    for(int j=0; j<npt+4; j++){
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      tex=new TLatex(0.3,40.08,"CMS PbPb #sqrt{s_{NN}}=2.76TeV");
      tex->SetTextSize(textsize*0.85);
      tex->Draw();
      tex=new TLatex(0.3,35.07,"|#eta|<0.8");
      tex->SetTextSize(textsize);
      tex->Draw();
    }
    //if (iw < 4) tex=new TLatex(ptit[iw][0]+2,35.07,htit[iw]);
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw],lymin[iw]*1.7,lxmax[iw],lymax[iw]*1.47);
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"#chi_{523} tight track cuts","p");
    }
    if (iw==1) leg->Draw();
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineColor(col);
      ge->Draw("pe");
      leg->AddEntry(ge,"#chi_{523} loose track cuts","p");
    }

  }
  cout << "end of process" << endl;
  can->cd();
  can->Print("./figures/chi523_pt_10cen_sysTrackCuts2760gev.png");
  can->Print("./figures/chi523_pt_10cen_sysTrackCuts2760gev.pdf");
}
