...
double lymin[Mpads],lymax[Mpads];
  ...
  if(iw==1){
    for (int im=0;im<1;im++)
    {
      ...
      ge->Draw("le");
      TLegend *leg = new TLegend(lxmin[iw]*0.2,lymin[iw]*0.6,lxmax[iw]*0.6,lymax[iw]*0.55*0.93);
      leg->SetFillStyle(0);
      leg->SetFillColor(0);
      leg->SetTextSize(textsize);
      leg->AddEntry(ge,"2.76 TeV, #eta/s=0.08","l");
      leg->Draw();
      tex=new TLatex(11,0.00032,"Gauss + hydro.");
      tex->SetTextSize(textsize*1.0);
      tex->Draw();
    }
    for (int im=0;im<1;im++)
...
//Error bar line style
  ...
  if(iw==4){
    for (int im=0;im<1;im++)
    {
      ...
      //ge->SetFillColor(kYellow+2);
      ge->Draw("le");
      TLegend *leg = new TLegend(lxmin[iw]*0.2,lymin[iw]*1.1,lxmax[iw]*0.6,lymax[iw]*0.97*0.96);
      leg->SetFillStyle(0);
      leg->SetFillColor(0);
      leg->SetTextSize(textsize);
      leg->AddEntry(ge,"2.76 TeV, #eta/s=0.08","l");
      leg->Draw();
      tex=new TLatex(11,0.0143,"Gauss + hydro.");
      tex->SetTextSize(textsize*1.0);
      tex->Draw();
    }
  }
    ...
    for (int im=0;im<1;im++)
    {
      ...
      int col=1;
      //if (ndp[j]==0) continue;
      ge=new TGraphErrors(10,&xa5[j][0],&ya5[j][0],&xe5[j][0],&ye5[j][0]);
  ...
  //can->Print("./figures/plot_vncent_theorycomp.gif");
}
