...
double nxa_osu000[Mfiles][Mpoints],nxe_osu000[Mfiles][Mpoints];
double nya_osu000[Mfiles][Mpoints],nye_osu000[Mfiles][Mpoints];
double nxa_osu000kln[Mfiles][Mpoints],nxe_osu000kln[Mfiles][Mpoints];
double nya_osu000kln[Mfiles][Mpoints],nye_osu000kln[Mfiles][Mpoints];
double nxa_osu008[Mfiles][Mpoints],nxe_osu008[Mfiles][Mpoints];
double nya_osu008[Mfiles][Mpoints],nye_osu008[Mfiles][Mpoints];
double nxa_osu008kln[Mfiles][Mpoints],nxe_osu008kln[Mfiles][Mpoints];
double nya_osu008kln[Mfiles][Mpoints],nye_osu008kln[Mfiles][Mpoints];

double yesys[Mfiles][Mpoints],ye5sys[Mfiles][Mpoints];
...
double lymin[Mpads],lymax[Mpads];
  ...
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt_osu; j++){
      inSP>>nya_osu000[i][j]; inSP>>nye_osu000[i][j];
      nxa_osu000[i][j]=osu_cent[j];
      nxe_osu000[i][j]=0.0;
    }
  }
  inSP.close();
  ifstream inSP;
  inSP.open("./osu_chiX_KLN_ideal_integral.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt_osu; j++){
      inSP>>nya_osu000kln[i][j]; inSP>>nye_osu000kln[i][j];
      nxa_osu000kln[i][j]=osu_cent[j];
      nxe_osu000kln[i][j]=0.0;
    }
  }
  inSP.close();
  ifstream inSP;
  ...
  inSP.close();
  ifstream inSP;
  inSP.open("./osu_chiX_KLN_SV008_integral.txt");
  for(int i=0; i<nw; i++){
    for(int j=0; j<npt_osu; j++){
      inSP>>nya_osu008kln[i][j]; inSP>>nye_osu008kln[i][j];
      nxa_osu008kln[i][j]=osu_cent[j];
      nxe_osu008kln[i][j]=0.0;
    }
  }
  inSP.close();
  ...
  for (int iw=0;iw<nw;iw++)
  {
    ...
    if (iw==0)
    {
      tex=new TLatex(11,2.5100,"CMS Preliminary");
    ...
    if (iw==3)
    {
      ...
      tex=new TLatex(5.6,4.55*1.1,"L. Yan and J.-Y. Ollitrault");
      tex->SetTextSize(textsize*0.88);
      //tex->Draw();
      tex=new TLatex(14.8,4.55*1.15*0.96,"iEBE-VISHNU");
      tex->SetTextSize(textsize*1.0);
      tex->Draw();
      tex=new TLatex(3.5,4.55*1.15*0.5,"Glb and KLN from iEBE-VISHNU");
      tex->SetTextSize(textsize*0.78);
      //tex->Draw();
    }
    if (iw==2)
    {
      tex=new TLatex(2.3,4.55*1.1,"J. Qian, U. Heinz and J. Liu");
      tex->SetTextSize(textsize*0.88);
      //tex->Draw();
      tex=new TLatex(15.8,4.55*1.15*0.96*0.98,"Viscous Hydro");
      tex->SetTextSize(textsize*1.0);
      tex->Draw();
    }
    //if (iw < 4) tex=new TLatex(ptit[iw][0]+2,0.27,htit[iw]);
    tex=new TLatex(ptit[iw][0]*13.7,ptit[iw][1]*0.451*(-1)*13.8,htit[iw]);
    ...
    // Legend
    //
    TLegend *leg = new TLegend(lxmin[iw]*1.1,lymin[iw]*1.1*0.87,lxmax[iw]*1.1,lymax[iw]*0.87);
    leg->SetFillStyle(0);
    leg->SetFillColor(0);
    leg->SetTextSize(textsize);
    TLegend *leg2 = new TLegend(lxmin[iw]*0.66*1.4,lymin[iw]*0.99*1.15*0.87,lxmax[iw]*1.0*1.4,lymax[iw]*0.91*1.15*0.87*0.9*1.05);
    leg2->SetFillStyle(0);
    leg2->SetFillColor(0);
    leg2->SetTextSize(textsize);
    TLegend *leg3 = new TLegend(lxmin[iw]*0.52,lymin[iw]*1.05*0.86*0.87*0.95*1.1,lxmax[iw]*1.0,lymax[iw]*0.91*0.87*1.05*1.1*0.87);
    leg3->SetFillStyle(0);
    leg3->SetFillColor(0);
    leg3->SetTextSize(textsize);
    TLegend *leg4 = new TLegend(lxmin[iw]*0.52,lymin[iw]*0.99*1.15*0.87,lxmax[iw]*1.0,lymax[iw]*0.91*1.15*0.87*0.9*0.94);
    leg4->SetFillStyle(0);
    leg4->SetFillColor(0);
...
//OSU
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineStyle(2);
      ge->SetLineColor(col);
      //ge->SetFillStyle(4000);
      ge->SetFillColor(kRed-10);
      ge->Draw("3same");
      leg3->AddEntry(ge,"#eta/s=0.08, Glb","l");
//Error bar line style
      ge2=new TGraphErrors(6,&nxa_osu008[j][0],&nya_osu008[j][0],&nxe_osu008[j][0],0);
      ge2->SetLineWidth(3);
      ge2->SetLineStyle(2);
      ge2->SetLineColor(col);
      ge2->Draw("lsame");
      //leg3->AddEntry(ge2,"Glb, #eta/s=0","l");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*5+iw;
      int col=6;
      //if (ndp[j]==0) continue;
      ge=new TGraphErrors(6,&nxa_osu008kln[j][0],&nya_osu008kln[j][0],&nxe_osu008kln[j][0],&nye_osu008kln[j][0]);
      ge->SetTitle("");
      //ge->SetMarkerStyle(25);
      //      ge->SetMarkerStyle(msty[im]);
      ge->SetMarkerSize(0.0001);
      ...
      ge->SetLineColor(col);
      //ge->SetFillStyle(4000);
      ge->SetFillColor(kMagenta-10);
      ge->Draw("e3same");
      leg3->AddEntry(ge,"#eta/s=0.08, KLN","l");
//Error bar line style
      ge2=new TGraphErrors(6,&nxa_osu008kln[j][0],&nya_osu008kln[j][0],&nxe_osu008kln[j][0],0);
      ge2->SetLineWidth(3);
      ge2->SetLineStyle(3);
      ge2->SetLineColor(col);
      ge2->Draw("lsame");
      ///leg3->AddEntry(ge2,"KLN, #eta/s=0","l");
    }
/*
//AMPT
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetLineStyle(1);
      ge->SetLineColor(col);
      //ge->SetFillColorAlpha(kBlue-10, 0.35);
      ///ge->SetFillStyle(4000);
      ge->SetFillColor(kBlue-10);
      ge->Draw("e3same");
      //leg2->AddEntry(ge,"2.76 TeV, AMPT","l");
//Error bar line style
      ge2=new TGraphErrors(7,&nxa_oli020[j][0],&nya_oli020[j][0],&nxe_oli020[j][0],0);
      ge2->SetLineWidth(3);
      ge2->SetLineStyle(1);
      ge2->SetLineColor(col);
      ge2->Draw("lsame");
...
//Oli
    for (int im=0;im<1;im++)
    {
      ...
      //ge->SetFillColor(kYellow+2);
      ge->Draw("pe");
      leg4->AddEntry(ge,"#eta/s=0.08, Gauss","l");
//Error bar line style
      ...
      ge2->SetLineColor(col);
      ge2->Draw("l");
      //leg4->AddEntry(ge2,"#eta/s=0.08, Gauss","l");
    }
    if (iw==2) leg4->Draw();
...
//OSU
    for (int im=0;im<1;im++)
    {
      ...
      int col=7;
      //if (ndp[j]==0) continue;
      ge=new TGraphErrors(6,&nxa_osu000[j][0],&nya_osu000[j][0],&nxe_osu000[j][0],&nye_osu000[j][0]);
      ge->SetTitle("");
      //ge->SetMarkerStyle(25);
      //      ge->SetMarkerStyle(msty[im]);
      //ge->SetMarkerSize(0.0001);
      ...
      ge->SetLineColor(col);
      ge->SetFillStyle(4000);
      ge->SetFillColor(kCyan-10);
      ///ge->Draw("3same");
      ///leg3->AddEntry(ge,"Glb, #eta/s=0","l");
//Error bar line style
      ge2=new TGraphErrors(6,&nxa_osu000[j][0],&nya_osu000[j][0],&nxe_osu000[j][0],0);
      ge2->SetLineWidth(3);
      ge2->SetLineStyle(2);
      ge2->SetLineColor(col);
      ///ge2->Draw("lsame");
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ge->SetFillColor(kRed-10);
      ///ge->Draw("e3same");
      ///leg3->AddEntry(ge,"Glb, #eta/s=0.2","pl");
//Error bar line style
      ge2=new TGraphErrors(6,&nxa_osu020[j][0],&nya_osu020[j][0],&nxe_osu020[j][0],0);
      ge2->SetLineWidth(3);
      ge2->SetLineStyle(7);
      ge2->SetLineColor(col);
      ///ge2->Draw("lsame");
      //leg3->AddEntry(ge2,"Glb, #eta/s=0.2","l");
    }
    for (int im=0;im<1;im++)
    {
      int j=im*5+iw;
      int col=6;
      //if (ndp[j]==0) continue;
      ge=new TGraphErrors(6,&nxa_osu000kln[j][0],&nya_osu000kln[j][0],&nxe_osu000kln[j][0],&nye_osu000kln[j][0]);
      ge->SetTitle("");
      //ge->SetMarkerStyle(25);
      //      ge->SetMarkerStyle(msty[im]);
      ge->SetMarkerSize(0.0001);
      ...
      ge->SetFillColor(kMagenta-10);
      ///ge->Draw("e3same");
      ///leg3->AddEntry(ge,"KLN, #eta/s=0","l");
//Error bar line style
      ge2=new TGraphErrors(6,&nxa_osu000kln[j][0],&nya_osu000kln[j][0],&nxe_osu000kln[j][0],0);
      ge2->SetLineWidth(3);
      ge2->SetLineStyle(3);
      ge2->SetLineColor(col);
      ///ge2->Draw("lsame");
    ...
    for (int im=0;im<1;im++)
    {
      ...
      ///ge->Draw("e3same");
      ///leg3->AddEntry(ge,"KLN, #eta/s=0.2","pl");
//Error bar line style
      ge2=new TGraphErrors(6,&nxa_osu020kln[j][0],&nya_osu020kln[j][0],&nxe_osu020kln[j][0],0);
      ge2->SetLineWidth(3);
      ge2->SetLineStyle(5);
      ge2->SetLineColor(col);
      ///ge2->Draw("lsame");
      //leg3->AddEntry(ge2,"KLN, #eta/s=0.2","l");
    }
    if (iw==3) leg3->Draw();
...
*/
   ...
   tex->SetLineWidth(2);
   //tex->Draw();
   TLatex *   tex = new TLatex(0.24,0.943,"0.3<p_{T}<3.0 GeV/c         |#eta|<0.8");
   tex->SetNDC();
   tex->SetTextFont(42);
   tex->SetTextSize(0.074);
   tex->SetLineWidth(2);
  ...
  //can->Print("./figures/plot_chicent_theorycomp.gif");
}
