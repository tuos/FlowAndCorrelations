...
nTables = 90
flow_str = '["FLOW", "MIXED HARMONICS"]'
flowvnn_str = '["FLOW", "HARMONICS"]'
response_str = '["NONLINEAR RESPONSE", "COEFFICIENTS"]'

print('')
for i in range(nTables):
    print('#####Adding Table '+str(i+1)+'#####')
    if i>=70 :
        print('table'+str(i+1)+' = Table("Figure 1 XX at 5.02 TeV")')
    else :
        print('table'+str(i+1)+' = Table("Figure 1 XX at 5.02 TeV in 20-60% centrality")')
    if i>=70 :
        print('table'+str(i+1)+'.description = "XX at 5.02 TeV as a function of centrality"')
    else :
        print('table'+str(i+1)+'.description = "XX at 5.02 TeV as a function of pT in the 20-60% centrality range"')
    print('table'+str(i+1)+'.location = "Data from Figure 1"')

    if i<20 :
        print('table'+str(i+1)+'.keywords["observables"] = '+flow_str)
    elif i>=20 and i<30 :
        print('table'+str(i+1)+'.keywords["observables"] = '+flowvnn_str)
    elif i>=30 and i<40 :
        print('table'+str(i+1)+'.keywords["observables"] = '+flow_str)
    elif i>=40 and i<60 :
        print('table'+str(i+1)+'.keywords["observables"] = '+response_str)
    elif i>=60 and i<70 :
        print('table'+str(i+1)+'.keywords["observables"] = '+flow_str)
    else :
        print('table'+str(i+1)+'.keywords["observables"] = '+response_str)
    print('table'+str(i+1)+'.keywords["reactions"] = ["Pb Pb --> CHARGED X"]') 
    print('data'+str(i+1)+' = np.loadtxt("../data_hin17005/fig1_a1.txt", skiprows=0)')
    if i>=60 :
        print('xValue'+str(i+1)+' = Variable("Centrality", is_independent=True, is_binned=True, units="%")')
    else :
        print('xValue'+str(i+1)+' = Variable("Transverse momentum", is_independent=True, is_binned=True, units="GeV/c")')

    print('xValue'+str(i+1)+'.digits = 2')
    print('xValue'+str(i+1)+'.values = list(zip(data'+str(i+1)+'[:,0], data'+str(i+1)+'[:,1]))')

    if i<20 :
        print('yValue'+str(i+1)+' = Variable("Mixed harmonics XX", is_independent=False, is_binned=False, units="")')
    elif i>=20 and i<30 :
        print('yValue'+str(i+1)+' = Variable("Flow harmonics XX", is_independent=False, is_binned=False, units="")')
    elif i>=30 and i<40 :
        print('yValue'+str(i+1)+' = Variable("Mixed harmonics XX", is_independent=False, is_binned=False, units="")')
    elif i>=40 and i<60 :
        print('yValue'+str(i+1)+' = Variable("Nonlinear response coefficients XX", is_independent=False, is_binned=False, units="")')
    elif i>=60 and i<70 :
        print('yValue'+str(i+1)+' = Variable("Mixed harmonics XX", is_independent=False, is_binned=False, units="")')
    else :
        print('yValue'+str(i+1)+' = Variable("Nonlinear response coefficients XX", is_independent=False, is_binned=False, units="")')
