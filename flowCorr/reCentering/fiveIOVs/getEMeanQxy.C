...
TH1D::SetDefaultSumw2();
  ...
  TFile* inFile = new TFile("../flowCorr_data_5020_v03Ana_5TevCentPar_sep25E_merged.root");
  ...
  ofstream outvhf;
  outvhf.open("outvmeanHF_v08_corrFlowE.txt");

  outvhf<<"double meanQxPlusE[nCentBin][nHarmonics] = {";
...
/////////////
  outvhf<<"double meanQyPlusE[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
///////////
  outvhf<<"double meanQxMinusE[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
////////////
  outvhf<<"double meanQyMinusE[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
/////////
  ...
  ofstream outvtrk;
  outvtrk.open("outvmeanTrk_v08_corrFlowE.txt");
...
/////////
  ...
  outvtrk<<"double meanQxTrkPtE[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
...
/////////
  ...
  outvtrk<<"double meanQyTrkPtE[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
