...
TH1D::SetDefaultSumw2();
  ...
  TFile* inFile = new TFile("../flowCorr_data_5020_v03Ana_5TevCentPar_sep25A_merged.root");
...
/*
  ofstream outvhf;
  outvhf.open("outvmeanHF_v08_corrFlowA.txt");

  outvhf<<"double meanQxPlusA[nCentBin][nHarmonics] = {";
...
/////////////
  outvhf<<"double meanQyPlusA[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
///////////
  outvhf<<"double meanQxMinusA[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
////////////
  outvhf<<"double meanQyMinusA[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
*/
  ...
  ofstream outvtrk;
  outvtrk.open("outvmeanTrk_v08_corrFlowA.txt");
...
/////////
  ...
  outvtrk<<"double meanQxTrkPtA[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
...
/////////
  ...
  outvtrk<<"double meanQyTrkPtA[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
