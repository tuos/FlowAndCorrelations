...
TH1D::SetDefaultSumw2();
  ...
  TFile* inFile = new TFile("../flowCorr_data_5020_v03Ana_5TevCentPar_sep25B_merged.root");
  ...
  ofstream outvhf;
  outvhf.open("outvmeanHF_v08_corrFlowB.txt");

  outvhf<<"double meanQxPlusB[nCentBin][nHarmonics] = {";
...
/////////////
  outvhf<<"double meanQyPlusB[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
///////////
  outvhf<<"double meanQxMinusB[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
////////////
  outvhf<<"double meanQyMinusB[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
/////////
  ...
  ofstream outvtrk;
  outvtrk.open("outvmeanTrk_v08_corrFlowB.txt");
...
/////////
  ...
  outvtrk<<"double meanQxTrkPtB[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
...
/////////
  ...
  outvtrk<<"double meanQyTrkPtB[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
