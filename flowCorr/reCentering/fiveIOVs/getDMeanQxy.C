...
TH1D::SetDefaultSumw2();
  ...
  TFile* inFile = new TFile("../flowCorr_data_5020_v03Ana_5TevCentPar_sep25D_merged.root");
  ...
  ofstream outvhf;
  outvhf.open("outvmeanHF_v08_corrFlowD.txt");

  outvhf<<"double meanQxPlusD[nCentBin][nHarmonics] = {";
...
/////////////
  outvhf<<"double meanQyPlusD[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
///////////
  outvhf<<"double meanQxMinusD[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
////////////
  outvhf<<"double meanQyMinusD[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
/////////
  ...
  ofstream outvtrk;
  outvtrk.open("outvmeanTrk_v08_corrFlowD.txt");
...
/////////
  ...
  outvtrk<<"double meanQxTrkPtD[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
...
/////////
  ...
  outvtrk<<"double meanQyTrkPtD[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
