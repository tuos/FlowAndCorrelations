...
TH1D::SetDefaultSumw2();
  ...
  TFile* inFile = new TFile("../flowCorr_data_5020_v02Ana_5tevReCentReal_mar17A_merged.root");
  for(int ibin=0; ibin<nCentBin; ibin++){
    for(int iH=0; iH<nHarmonics; iH++){
      ...
      hQhfPlusQ[ibin][iH] = (TH1D*)inFile->Get(Form("flowCorr/hQhfPlusQ_ibin%d_iH%d",ibin,iH));
      ...
      hQhfMinusQ[ibin][iH] = (TH1D*)inFile->Get(Form("flowCorr/hQhfMinusQ_ibin%d_iH%d",ibin,iH));
      hQtrkPlusX[ibin][iH] = (TH1D*)inFile->Get(Form("flowCorr/hQtrkPlusX_ibin%d_iH%d",ibin,iH));
      hQtrkPlusY[ibin][iH] = (TH1D*)inFile->Get(Form("flowCorr/hQtrkPlusY_ibin%d_iH%d",ibin,iH));
      hQtrkMinusX[ibin][iH] = (TH1D*)inFile->Get(Form("flowCorr/hQtrkMinusX_ibin%d_iH%d",ibin,iH));
      hQtrkMinusY[ibin][iH] = (TH1D*)inFile->Get(Form("flowCorr/hQtrkMinusY_ibin%d_iH%d",ibin,iH));
  ...
  ofstream outvhf;
  outvhf.open("outvmeanHF_v08_corrFlow.txt");

  outvhf<<"double meanQxPlus[nCentBin][nHarmonics] = {";
...
/////////////
  outvhf<<"double meanQyPlus[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
///////////
  outvhf<<"double meanQxMinus[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
////////////
  outvhf<<"double meanQyMinus[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvhf<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
...
/////////
  ...
  ofstream outvtrk;
  outvtrk.open("outvmeanTrk_v08_corrFlow.txt");

  outvtrk<<"double meanQxTrkPlus[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<hQtrkPlusX[ibin][iH]->GetMean()<<", ";
    }
    outvtrk<<hQtrkPlusX[ibin][nHarmonics-1]->GetMean()<<"}, "<<endl;
  }
  outvtrk<<"{";
  for(int iH=0; iH<nHarmonics-1; iH++){
    outvtrk<<hQtrkPlusX[nCentBin-1][iH]->GetMean()<<", ";
  }
  outvtrk<<hQtrkPlusX[nCentBin-1][nHarmonics-1]->GetMean()<<"}}; "<<endl;
/////////////
  outvtrk<<"double meanQyTrkPlus[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<hQtrkPlusY[ibin][iH]->GetMean()<<", ";
    }
    outvtrk<<hQtrkPlusY[ibin][nHarmonics-1]->GetMean()<<"}, "<<endl;
  }
  outvtrk<<"{";
  for(int iH=0; iH<nHarmonics-1; iH++){
    outvtrk<<hQtrkPlusY[nCentBin-1][iH]->GetMean()<<", ";
  }
  outvtrk<<hQtrkPlusY[nCentBin-1][nHarmonics-1]->GetMean()<<"}}; "<<endl;
///////////
  outvtrk<<"double meanQxTrkMinus[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<hQtrkMinusX[ibin][iH]->GetMean()<<", ";
    }
    outvtrk<<hQtrkMinusX[ibin][nHarmonics-1]->GetMean()<<"}, "<<endl;
  }
  outvtrk<<"{";
  for(int iH=0; iH<nHarmonics-1; iH++){
    outvtrk<<hQtrkMinusX[nCentBin-1][iH]->GetMean()<<", ";
  }
  outvtrk<<hQtrkMinusX[nCentBin-1][nHarmonics-1]->GetMean()<<"}}; "<<endl;
////////////
  outvtrk<<"double meanQyTrkMinus[nCentBin][nHarmonics] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<hQtrkMinusY[ibin][iH]->GetMean()<<", ";
    }
    outvtrk<<hQtrkMinusY[ibin][nHarmonics-1]->GetMean()<<"}, "<<endl;
  }
  outvtrk<<"{";
  for(int iH=0; iH<nHarmonics-1; iH++){
    outvtrk<<hQtrkMinusY[nCentBin-1][iH]->GetMean()<<", ";
  }
  outvtrk<<hQtrkMinusY[nCentBin-1][nHarmonics-1]->GetMean()<<"}}; "<<endl;
/////////

  outvtrk<<"double meanQxTrkPt[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
...
/////////
  ...
  outvtrk<<"double meanQyTrkPt[nCentBin][nHarmonics][nPtBin] = {";
  for(int ibin=0; ibin<nCentBin-1; ibin++){
    outvtrk<<"{";
    for(int iH=0; iH<nHarmonics-1; iH++){
      outvtrk<<"{";
